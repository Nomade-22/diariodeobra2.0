...
export function bindCheckout(context){
  ...
  btn.addEventListener('click', ()=>{
    ...
    const rec = { id:uid(), team, driver, job, vehicle, kmStart:km, timeOut, obs, photos: context.outPhotos.slice(), tools: selected, createdAt: new Date().toISOString(), createdBy: { name:user.name, id:user.id, role:user.role } };
    outs.unshift(rec); write(LS.outs, outs);

    context.currentReturn = { id:rec.id, timeIn:new Date().toISOString(), kmEnd:'', notes:'', photos:[], checklist: selected.map(i=>({ ...i, status:'voltou', condition:'ok', notes:'' })), createdBy: { name:user.name, id:user.id, role:user.role } };

    alert('Saída registrada!');
    context.outPhotos.length = 0; const pc=document.getElementById('outPhotoCount'); if(pc) pc.textContent='Nenhuma foto';
    document.getElementById('outDriver').value=''; document.getElementById('outVehicle').value=''; document.getElementById('outKm').value=''; document.getElementById('outObs').value=''; document.getElementById('outTime').value=new Date().toISOString().slice(0,16);
