...
export function renderTeams(refresh){
  ...
  ul.innerHTML = teams.map((t,i)=>`
    <li style="display:flex;gap:8px;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border)">
      <span>${t}</span>
      <span>
        ${admin ? `
  ...
  `).join('') || '<div class="small">Nenhum funcionário cadastrado.</div>';

  if(!admin) return;

  ul.querySelectorAll('[data-del-team]').forEach(b=>{
  ...
  ul.querySelectorAll('[data-edit-team]').forEach(b=>{
    b.addEventListener('click', ()=>{
      const i = +b.dataset.editTeam;
      const nv = prompt('Novo nome do funcionário:', teams[i]);
      if(nv && nv.trim()){
        teams[i] = nv.trim();
        write(LS.teams, teams);
        refresh();
...
export function renderJobs(refresh){
  ...
  `).join('') || '<div class="small">Nenhuma obra/cliente cadastrado.</div>';

  if(!admin) return;

  ul.querySelectorAll('[data-del-job]').forEach(b=>{
    b.addEventListener('click', ()=>{
      const i = +b.dataset.delJob;
      jobs.splice(i,1);
      write(LS.jobs, jobs);
      refresh();
    });
  });
  ul.querySelectorAll('[data-edit-job]').forEach(b=>{
    b.addEventListener('click', ()=>{
      const i = +b.dataset.editJob;
      const nv = prompt('Novo nome da obra/cliente:', jobs[i]);
      if(nv && nv.trim()){
        jobs[i] = nv.trim();
        write(LS.jobs, jobs);
        refresh();
...
export function renderTools(afterRender){
  ...
  list.innerHTML = tools.map((t,i)=>{
    if(!t.id) t.id = uid();
    return `
      <div class="gridrow" style="grid-template-columns:40px 1fr 160px 100px 1fr;gap:10px">
        <div>${admin ? `<button class="btn danger" data-del-tool="${i}" title="Excluir">✕</button>` : ''}</div>
        ...
        <div><input ${admin?'':'readonly'} data-qty="${i}" type="number" min="0" value="${t.qty??0}" /></div>
        <div><input ${admin?'':'readonly'} data-obs="${i}" value="${t.obs||''}" placeholder="Obs"/></div>
      </div>
    `;
  }).join('') || '<div class="small">Nenhuma ferramenta cadastrada.</div>';

  if(!admin) return; // só Admin pode alterar

  // handlers de edição
  list.querySelectorAll('[data-name]').forEach(inp=> inp.addEventListener('input',()=>{
    const i=+inp.dataset.name; tools[i].name = inp.value; write(LS.tools, tools); afterRender&&afterRender();
  }));
  list.querySelectorAll('[data-code]').forEach(inp=> inp.addEventListener('input',()=>{
    const i=+inp.dataset.code; tools[i].code = inp.value; write(LS.tools, tools); afterRender&&afterRender();
  }));
  list.querySelectorAll('[data-qty]').forEach(inp=> inp.addEventListener('input',()=>{
    const i=+inp.dataset.qty; tools[i].qty = +inp.value||0; write(LS.tools, tools); afterRender&&afterRender();
  }));
  list.querySelectorAll('[data-obs]').forEach(inp=> inp.addEventListener('input',()=>{
    const i=+inp.dataset.obs; tools[i].obs = inp.value; write(LS.tools, tools); afterRender&&afterRender();
  }));

  // excluir
  list.querySelectorAll('[data-del-tool]').forEach(btn=> btn.addEventListener('click',()=>{
    const i = +btn.dataset.delTool;
    tools.splice(i,1);
    write(LS.tools, tools);
    renderTools(afterRender);
    afterRender && afterRender();
  }));
}

/* ===== Picker de Ferramentas na Saída ===== */
...
export function renderPicker(pickState){
  ...
  list.querySelectorAll('[data-pick]').forEach(ch=> ch.addEventListener('change',()=>{
  ...
  list.querySelectorAll('[data-take]').forEach(inp=> inp.addEventListener('input',()=>{
