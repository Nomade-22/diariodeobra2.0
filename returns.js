...
export function bindReturn(context){
  ...
  sel.addEventListener('change',(e)=>{
    ...
    context.currentReturn = { id:o.id, timeIn:new Date().toISOString(), kmEnd:'', notes:'', photos:[], checklist: o.tools.map(i=>({ ...i, status:'voltou', condition:'ok', notes:'' })) };
  ...
  const btn = document.getElementById('btnFinishReturn'); if(btn){ btn.addEventListener('click', ()=>{
    ...
    context.currentReturn=null; context.retPhotos.length=0; const pc=document.getElementById('retPhotoCount'); if(pc) pc.textContent='Nenhuma foto'; document.getElementById('retList').innerHTML=''; const rm=document.getElementById('retMissing'); if(rm) rm.textContent=''; context.refreshOpenOuts();
  }); }
}
